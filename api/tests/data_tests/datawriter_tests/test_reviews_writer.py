...
from api.data.common import APPROVED, INSUFFICIENT
...
class ReviewsWriterTest(LoadersWritersBaseTest):
    ...
    def test_approve_review(self, mock_datetime):
        VALID_REVIEW_ID = 5
        self.assertEqual(approve_review(VALID_REVIEW_ID), VALID_REVIEW_ID)
        self.cur.execute(
            'SELECT type FROM flag WHERE flag.review_id = %s',
            VALID_REVIEW_ID
        )
        flag = self.cur.fetchone()
        self.assertEqual(flag['type'], APPROVED)

    def test_reject_review(self, mock_datetime):
        VALID_REVIEW_ID = 6
        self.assertEqual(reject_review(VALID_REVIEW_ID), VALID_REVIEW_ID)
        self.cur.execute(
            'SELECT * FROM flag WHERE flag.review_id = %s',
            VALID_REVIEW_ID
        )
        flag = self.cur.fetchone()
        self.assertEqual(flag['type'], INSUFFICIENT)
